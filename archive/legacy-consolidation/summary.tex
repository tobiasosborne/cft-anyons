...
Fibonacci CFT is invoked, following Chat~2's stated choice.\unchecked\footnote{No local source for $(G_2)_1$ chiral CFT data; see \texttt{RESEARCH\_NOTES.md} acquisition-queue entry \texttt{aq:g2-1-chiral-cft} for the references to acquire.}\
...
\item Koo, Saleur: lattice Virasoro approximants from XXZ/RSOS / TL chains
      (chat~1 §10).\unchecked\footnote{Literature DB has paper\#590 (2017 follow-up applying the Koo--Saleur formula), but no local PDF of the original 1994 Koo--Saleur paper; see \texttt{RESEARCH\_NOTES.md} acquisition-queue entry \texttt{aq:koo-saleur} for the original paper to acquire.}
...
\item Pasquier: ADE lattice models / RSOS height models (chat~1 §1).\unchecked\footnote{No local source; see \texttt{RESEARCH\_NOTES.md} acquisition-queue entry \texttt{aq:pasquier} for the paper to acquire.}
\item Huse: identification of ABF critical points with the Friedan--Qiu--Shenker
      unitary minimal series $M(m, m+1)$ (chat~1 §13).\unchecked\footnote{No local source; see \texttt{RESEARCH\_NOTES.md} acquisition-queue entry \texttt{aq:huse} for the paper to acquire.}
\item Fjelstad, Fuchs, Runkel, Schweigert (``FRS''): full RCFT correlators
      ...
      $A = \one$ in the Cardy case (chat~1 §1).\unchecked\footnote{No local source; see \texttt{RESEARCH\_NOTES.md} acquisition-queue entry \texttt{aq:frs} for the papers to acquire.}
...
\item ``Anyonic MERA literature'': charge-compatible MERA tensors,
      scaling-superoperator language (chat~1 §1.4).\unchecked\footnote{No local source; see \texttt{RESEARCH\_NOTES.md} acquisition-queue entry \texttt{aq:anyonic-mera} for the papers to acquire.}
