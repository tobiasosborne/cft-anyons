...
| **P1.6** | Add `CONVENTIONS.md` entries for: (a) **vacuum index convention** (decide: 0-indexed vs 1-indexed; canonical: MMA-compatible `1` for numerical code, categorical `1 ∈ Irr(C)` as the abstract statement); (b) **F-matrix gauge** (declare unitary as canonical, document involutory translation rule for TensorCategories.jl-derived content); (c) **dagger/adjoint convention** (matches `summary.tex` Convention 2.1); (d) **multiplicity index notation + multiplicity-free assumption** (matches Def 2.5; document that all currently-used categories — Fibonacci, Ising, sVec — are multiplicity-free, so the MMA bug LB-1 is latent not active); (e) **complex conjugation symbol**; (f) **TikZ macro source** (which file is canonical); **(g) particle-number disambiguation** — `L` = tensor degree / lattice length / cell count vs `n` = count of non-vacuum legs; document that summary.tex's `H_N^W` uses `N` for `L`, MMA uses `N` for `n`; **(h) local cell object `Q` choice** — five named options from summary.tex Remark 3.2.4 (`Q_full`, `Q_restr`, multiplicity-decorated, site-dependent `A_I`, coarse-grained `Q^⊗k`); declare `Q_full = ⊕_a a` as canonical default; document MMA's "absorbed vacuum" convention as equivalent; **(i) fusion-tree basis ordering** — left-associated (all three formulations already use this; lock it); **(j) empty-configuration / `N = 0` boundary** — `H_∅^W = δ_{W,1} · ℂ` (the conventional identification across all three formulations). Items (g)–(j) added per P1.4-early Opus deep-dive findings (`stocktake/reports/opus-hilbert-bridge.md` §4). | D: each convention declared with reasoning. |
| **P1.7** | Add GLOSSARY translation map entries for **CAD Lean** finite-coordinate skeleton vs `summary.tex` categorical statements. For each `summary.tex` definition that CAD Lean has a finite skeleton for (e.g., `H_N^W` Def 2.4 ↔ CAD `IndefiniteParticleSectorCoordinates`), record the translation. Use `reports/cad-lean.md` §5 (which already has a file-by-file mapping) as the source. **Per P1.4-early findings**: clarify scope per-file — note that the Lean side is at the *coordinate skeleton* level (not categorical) and document what the user must supply (e.g., `sectorBasis : Config label n → Type`) to instantiate it as a model of `H_N^W`. | M: each CAD Lean file mentioned in `reports/cad-lean.md` has a GLOSSARY translation entry. D. |
...
| Step | Action | Validation |
|---|---|---|
| **P8.1** | **Definitional bridge audit (Opus)**. Spawn an Opus agent to read MMA's `src/MobileAnyons/{config,hilbert,fsymbols,operators,interaction,braiding,paircreation,finegraining,wavelets,virasoro}.jl` and confirm: (a) every fundamental concept maps to a GLOSSARY entry, (b) the involutory F-matrix gauge convention is correctly handled (this is what MMA's `interaction.jl` and `braiding.jl` Hermitian-projector workaround does), (c) any divergence from CONVENTIONS.md (vacuum index, etc.) is documented. **Pre-fed inputs** (P1.4-early already specified the bridge): the seven conventions in `stocktake/reports/opus-hilbert-bridge.md` §4 are pre-flagged; the auditor's job here is to confirm each is correctly applied throughout MMA, not to rediscover them. Also flag any locations where the multiplicity-free assumption (LB-1, `RESEARCH_NOTES.md`) leaks beyond `enumerate_fusion_trees`. Output to `stocktake/reports/opus-mma-julia-bridge.md`. | M, R. |
...
| **P8.4** | Per-file import. `src/MobileAnyons/MobileAnyons.jl` (the module shell), then `config.jl`, then `hilbert.jl`, then `fsymbols.jl`. After each: `julia --project=. -e 'using MobileAnyons'`. **For `hilbert.jl`**: add `# TODO LB-1: multiplicity-free assumption; see RESEARCH_NOTES.md` marker above `enumerate_fusion_trees` line 58 (per P1.4-early finding §3.5). | M: import succeeds. D: file's exported symbols all have GLOSSARY entries. |
