...
\textbf{Status: sourced template plus local coefficient derivation.}  The
...
scalar symbol is a local algebraic calculation under CONVENTIONS.md (j).
...
% Source: literature/md/2010.11121/2010.11121.md:648 -- continuum one-particle Hilbert space and dispersion.
...
\]
on the Brillouin zone, with a separate zero-mode policy in the massless case.
For the massive shards below we assume \(\omega_\epsilon(k)>0\) on the patch in
which the one-particle formulas are used.
...
For \(\omega>0\), the oscillator complex structure is
\[
...
The compiler-level output for the scalar tier is therefore:
\[
\begin{array}{ll}
\mathfrak h_\epsilon
  & \text{one-particle momentum Hilbert space on the chosen Brillouin zone},\\
h_\epsilon
...
For the scalar Gaussian block, all later candidate generators can be constructed
first on the one-particle smooth momentum core and then second-quantized.  The
