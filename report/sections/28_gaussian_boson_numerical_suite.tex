...
The current numerical surface is deliberately narrow: it checks finite-periodic
symbol values, finite stiffness spectra, assigned Fourier eigenvectors,
periodic minima counts, and the implemented coefficient residuals/Hessians.  It
also checks centered periodic momentum labels for branch-aware low-energy
windows and test-local central finite-difference boost-time derivative oracles.
It does not test finite boost generators, finite rotation generators,
...
% Checked: src/GaussianBosons.jl:9--25 and :97--136; src/GaussianBosonNumerics.jl:88--203.
% Checked: test/runtests.jl:151--192, :209--304, and :306--372.
...
Hamiltonian claim; the physical Gaussian coefficient checks remain real
symmetric.
...
low-energy Hessian residual against zero.  The boost-time helper is also
checked on non-Klein-Gordon finite-range symbols by the independent
finite-difference oracle above.
...
\[
\begin{array}{ll}
1.&\text{sampled symbol residuals on shrinking low-energy windows},\\
2.&\text{comparison of open-boundary first moments with periodic symbol tests},\\
3.&\text{structural coefficient validation and positivity witnesses}.
\end{array}
\]
