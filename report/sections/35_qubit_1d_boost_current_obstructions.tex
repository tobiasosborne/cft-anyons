...
\[
  i[H,P]=0.
\]
For the local candidate \(P=\sum_j C_j\), \(C_j=i[h_j,h_{j+1}]\), the minimal
bulk density after the Jacobi cancellation of outer-overlap terms is
\[
...
\[
  A_j=u_j-u_{j+1}
\]
for a finite density \(u\).  CA-39 writes this and the next boost relation
\(i[P,K]=v^2H\) as explicit Pauli-coefficient equations.
...
nonzero.  The CA-39 boost equation then tests the remaining relation after the
allowed density-divergence and scalar-energy subtraction have been fixed.
