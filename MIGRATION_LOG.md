...
| P1.5     | 2026-05-16 | (this)   | Write explicit Hilbert-space translation maps (def:HP / def:Hlatt / def:indlim / def:mobile-Fock) | M, D, R | Opus (APPROVE WITH MINOR EDITS, 1 applied) | Transcribed bidirectional bijections from bridge §2.1-§2.3 into four Translation map fields; def:HP MMA = special case (partition strictly more general for per-cell A_I + W superselection); def:Hlatt MMA = equivalent under vacuum-leg absorption; def:indlim MMA = discrete fixed-L analogue (no continuum counterpart in MMA); def:mobile-Fock filled all three summary.tex pairings. CAD-side stays TBD pending P1.7. Review at stocktake/reports/opus-glossary-p1.5-review.md |
