...
| P1.6     | 2026-05-16 | (this)   | CONVENTIONS.md populated with 10 entries (vacuum index, F-gauge, dagger, multiplicity-free, complex conjugation, TikZ-deferred, particle-number, Q choice, fusion-tree ordering, N=0 boundary) | D, R | Opus (APPROVE WITH MINOR EDITS, 5 polish applied) | Letter-keyed entries (a)-(j) matching MIGRATION_PLAN P1.6; all 7 in-use forward-pointers from GLOSSARY resolve; (f) TikZ deferred (no TikZ in summary.tex, pre-registered hook). Both load-bearing CLAUDE.md hallucination callouts (vacuum-index STL-1; F-gauge involutory-vs-unitary) explicitly tagged. Review at stocktake/reports/opus-conventions-p1.6-review.md |
