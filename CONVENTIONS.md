...
## (aa) Coset Hamiltonian and scalar-augmented OAR candidates (2026-09-22)
**Choice:** `b>=0` is an integer auxiliary-energy cutoff. `V_b` denotes
the span of coset vacuum-descendant words of total grade at most `b`,
inside the existing filled-sea representation; `P_b` is its orthogonal
projection. Exact sea inclusions induce `J_Bb:V_b->V_B` for `B>=b`.
The proposed finite algebra is `A_b=B(V_b) direct-sum C`, with independent
scalar coordinate `lambda`, and refinement
`alpha_Bb(a,lambda)=(J_Bb a J_Bb^*+lambda*(1-J_Bb J_Bb^*),lambda)`.
The state is `omega_b(a,lambda)=<Omega,a Omega>`. This scalar is NOT a
state-dependent character `omega_b(a)`; that replacement would yield only
a CP completion. A candidate Hamiltonian is `(E|V_b,b+1)`.
The ambient penalized version, when separately used, is
`P_b E P_b+(b+1)*(1-P_b)`; statements about its ambient resolvent limit
must not conflate it with the vacuum-module operator.

For a source-controlled general construction choose `R_b=2b+1` and
`M_b=16(b+1)`. These are analytic formulas only; finite sparse witnesses
remain `M=8,16`, `k=1,2`, `b<=3`, using the proved smaller vacuum windows.
All Gram/spanning calculations use exact arithmetic and retain their metric;
no unrecorded numerical rank tolerance is introduced.
As a negative-control occupation state at `k=1`, fill flavor 1 at every
momentum in both copies and leave flavor 2 empty. Any formula for its
raw coset energy is a derivation/test target, not an assumed ground energy.
**Reasoning:** independent scalar augmentation may supply actual unital
finite algebra maps and positive dynamics; it does not establish spatial
finite-range locality of descendant projections or encode a generic category.
**Source:** local derivation targets `cft_machine/research/coset_hamiltonian.md`
and `cft_machine/research/coset_observable_system.md`; prior CR-2/CR-3 and
OAR conventions (x),(z).
The implemented low-grade descendant basis is ordered as `Omega`, then
`C_-2 Omega` when `b>=2`, then `C_-3 Omega` when `b>=3`; at `b=1`
only the vacuum remains. Coordinates retain the exact Gram matrix `G_b`.
Adjoints in these coordinates are `a^sharp=G_b^-1 a^* G_b` and
`J_Bb^sharp=G_b^-1 J_Bb^* G_B`. Scalar-augmentation tests use these
metric adjoints; ordinary conjugate transpose is not substituted silently.
General ordered-word spans/pivoting are a construction formula, not an
implemented large-grade enumeration in this first low-grade witness.

## (ab) Smooth coset smearings and bounded local generators (2026-09-22)
**Choice:** `fhat_n=(2*pi)^(-1) integral f(x) exp(-i*n*x) dx`;
real `f` obeys `fhat_-n=conj(fhat_n)`. A microscopic smooth-stress
approximation is `T_M(f)=sum_(|n|<=K_M) fhat_n C_(M,n,R_M)` with
`K_M=floor(M/16)`, `R_M=floor(M/8)`. These independent cutoff schedules
require the CR-2 safe-window proof for every mode used.
The finite descendant-space compression is
`T_b(f)=P_b T(f)P_b=sum_(|n|<=b) fhat_n P_b C_n P_b`.
Its unitary algebra element is `(exp(i*t*T_b(f)),1)`; in the vacuum-module
representation it acts as identity off `V_b`. Local nets use real smooth
interval-supported `f` and `t in Q`, with extension to real t through
proved strong continuity. Ambient coset direct sums and the vacuum-module
projective Diff representation remain distinct.

For the diagnostic sequence-algebra comparison only, use uniformly bounded
sequences in a common represented Hilbert space converging strongly with
their adjoints, supremum norm, and the strong-limit *-map. Its null ideal
is strong*-null, not the norm-null ideal of the registered soft-OAR source;
this diagnostic is not automatically an OAR construction.
**Source:** proposed constructions in `coset_observable_system.md` and
`smeared_coset_limits.md`; registered Carpi--Weiner energy-bound/core source
under `references/cft-machine/coset-symmetry/`; CR-2 and CONVENTIONS (z).

For the second-pass Ising locality audit, `L(I)` denotes represented
weak-operator limits of uniformly operator-norm-bounded sequences of even
finite-lattice observables supported in interval `I`, all embedded by the
same sharp CAR refinement. The local maximal limit algebra here is the
von Neumann algebra generated by `L(I)`. This uses WOT limits only to
identify local algebras; it does not declare WOT convergence multiplicative
or change the strict OAR connecting maps.

## (ac) Spacetime tile dictionary for anyonic chains (2026-09-23)
**Choice:** A nearest-neighbour Hamiltonian on the site-object chain of (r) is
written `H = -sum_j h_j`, `h_j in End_C(O tensor O)`, and expanded in a basis
of **elementary tiles**: morphism diagrams on two sites (vacancy projectors
`P_00, P_X0, P_0X, P_XX`; cup `b_j` and cap `d_j` of CA-66 with the raw
normalisation of (r); hops `h_j^+ : |X 1> -> |1 X>`, `h_j^- = (h_j^+)^dagger`;
the cup-cap `e_j = b_j d_j`; and, for categories with `N_{XX}^X != 0`, the
vertex tiles `nu, nu^dagger` between `|XX -> X>` and `|X1>, |1X>`, whose
normalisation is NOT YET FIXED and must be recorded before first use).
Imaginary time runs upward; Trotter is **brick-wall** with odd bonds first,
step `delta tau`, `beta = M delta tau`. A **history** assigns one tile (or
the identity) to each plaquette; its amplitude is the C-evaluation of the
composed diagram times the product of tile weights (CA-86, Lemma 86.1).
**Dilute nine-tile identification:** with in-nodes on the left and bottom
edges of a tile (sites `j, j+1`) and out-nodes on the top and right, the
source numbering of the dilute O(n) tiles maps to `1 = P_00`, `2,3 = P_X0,
P_0X`, `8 = P_XX`, `9 = e_j`, `4 = d_j` (cap), `5 = b_j` (cup), `6,7 = h_j^+-`.
The identification is pinned by `u = 0`, where the face operator must be the
identity (`rho_{1,2,3,8}(0) = 1`, all others 0). Loop fugacity is identified
with the quantum dimension, `n = beta_dTL = d_X`, consistent with the raw
cup-cap = `d_X` of (r); for dilute `A_L` type input `n = 2 cos(pi/(L+1))` and
`lambda` is one of the two branch solutions of `n = -2 cos 4 lambda`.
**Reasoning:** the dictionary makes the Levin--Wen rules (2)--(4) automatic
(they are properties of the C-evaluation) and isolates rule (1), isotopy, as
the only thing the tile weights break; brick-wall order is the choice that
makes the plaquettes compose as a planar diagram. Which of tiles 4/5 is cup
versus cap and which hop is `h^+` is a labelling; reflection symmetry
(`rho_4 = rho_5`, `rho_6 = rho_7`) makes the Hamiltonian independent of it.
**Source:** `references/lattice-symmetry/DiluteA22Strip2022/source/stripA22.tex:669--745`
(nine tiles, direction of action); `references/lattice-symmetry/ZhouBatchelor1997/source/9611156.tex:105--145`
